...
export const useHistoryStore = defineStore('history', {
  ...
  actions: {
    ...
    removeHistory(historyId) {
      this.history = this.history.filter(item => (item.historyId || item.id) !== historyId);
      this.saveHistory();
    },
    ...
    async removeHistoryApi(historyId) {
      ...
      try {
        console.log('删除浏览历史API：开始请求，historyId:', historyId);
    ...
    async getHistoryListApi() {
      ...
      try {
        ...
        if (response.data.code === 200) {
          ...
          console.log('获取浏览历史API：成功获取记录', historyList);
